...
Indexer: periodic scan first; live watch later. Waveforms and heavy analysis are on-demand, not a full-library import. The catalog API is `POST /catalog/scan` and `GET /catalog/entries?path=`. Audio is `GET /audio/{path}` (the catalog id is the file’s relative path) with HTTP range requests; bytes are read from disk. Folders are derived from indexed file paths. The Svelte UI browses those listings and previews via the range URL; the API allows browser origins (LAN-first, no auth).
