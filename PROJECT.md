...
Indexer: periodic scan first; live watch later. Startup and `POST /catalog/scan` run the indexer in the background so the API is available immediately. Clients watch `WS /catalog/scan` for `{status, done, total}` (`running` or `idle`). Rescans skip audio reads when size and mtime still match the catalog, but still refresh BPM, key, and tags inferred from the filename and path (`*_inferred` and inferred tags only; user corrections stay). Loop vs one-shot also uses stored duration and, on new or changed wav/aiff files between 0.6s and 60s, a subprocess call to loop-tempo-estimator (GPL CLI, not linked into FastAPI). Waveforms and other heavy analysis stay on-demand. The catalog API is `POST /catalog/scan` (starts a scan; `started` or `running` if one is already in flight), `WS /catalog/scan` (progress), `GET /catalog/entries?path=` (folder children), and `GET /catalog/files` (flat list with `limit`/`offset`/`prefix`/`sort`/`q`/`tag`/`key`/`bpm`/`path`/`recent`). File payloads include effective `bpm` and `key` (`user` if set, otherwise `inferred`) and effective `tags` (inferred minus user hides, plus user adds). Repeated `tag=` filters are AND. `key` is a pitch-class root (`C` matches `C` and `Cm`). `bpm` is a range id (`70-90`, `90-110`, `110-130`, `130-150`, `150+`; min inclusive, max exclusive). Repeated `path=` restricts the list to those catalog ids, or to ids that end with that path (so a cache copy of `Samples/Pack/kick.wav` still matches if the cache relative path is `Pack/kick.wav`). The desktop client uses this for cached files. `recent=true` keeps files whose catalog mtime is within the last 7 days. `q` is a case-insensitive substring match on the file’s relative path. Audio is `GET /audio/{path}` (the catalog id is the file’s relative path) with HTTP range requests; bytes are read from disk. Folders are derived from indexed file paths. The Svelte UI loads the file list in chunks as the user scrolls, and previews via the range URL. Form, role, and function filters are independent daisyUI radio groups and map to those `tag=` params. Key and BPM selects map to `key=` and `bpm=`. Sidebar Locations are All Samples, Downloaded (`location=downloaded`, cache hits only), and Recently Added (`location=recent`, mtime within 7 days). CORS allows configured UI origins (default: the Vite dev server). Tauri downloads a complete file to `~/Armarium/cache/{server relative path}` and only that cached file can be dragged into a DAW.
...
**Later / ideas** — collections, BPM/key editing, similarity, cache size caps, multi-user, mobile, internet access. Not in scope until the drag-to-DAW loop works.
